    ...
    <div class="container mx-auto mt-6">
        ...
        <nav class="mb-4 mx-5" aria-label="Breadcrumb">
            <ol class="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <li>
                    <span class="text-gray-500">Edit Participant</span>
                </li>
            </ol>
        </nav>
        ...
        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg overflow-hidden mx-5">
            ...
            <div class="flex justify-between items-center mb-6 mx-8 my-2">
                <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-200 mx-2">Edit Participant</h1>
                <a href="{{ route('participant.index') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-6 py-2 rounded-lg inline-block transition duration-200 ease-in-out">
                    <i class="fas fa-arrow-left mr-2"></i> Back to Participant List
                </a>
            </div>

            <!-- Edit Form -->
            <form action="{{ route('participant.update', $participant) }}" method="POST" class="space-y-4 mx-8 my-6">
                @csrf
                @method('PUT')

                <!-- Participant Name -->
                <div>
                    <label for="participant_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Participant Name</label>
                    <input type="text" id="participant_name" name="participant_name" value="{{ old('participant_name', $participant->participant_name) }}" required
                ...
                <!-- Select Dojo -->
                <div>
                    <label for="dojo_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Dojo</label>
                    <select id="dojo_id" name="dojo_id" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-gray-200" required>
                        ...
                        @foreach($dojos as $dojo)
                            <option value="{{ $dojo->id }}" {{ old('dojo_id', $participant->dojo_id) == $dojo->id ? 'selected' : '' }}>{{ $dojo->dojo_name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label for="participant_description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Description</label>
                    <textarea id="participant_description" name="participant_description"
                        class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-gray-200">{{ old('participant_description', $participant->participant_description) }}</textarea>
