                ...
                <table id="categoris-table" class="min-w-full bg-white dark:bg-gray-800 table-auto">
                    <thead class="bg-gray-100 dark:bg-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-sm font-medium text-gray-600 dark:text-gray-200">Nama Categori</th>
                    ...
                    <tbody>
                        @foreach($categoris as $categori)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-200">{{ $categori->category_name }}</td>
                                <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-200">{{ $categori->created_at->format('d M Y') }}</td>
                                <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-200">{{ $categori->updated_at->format('d M Y') }}</td>
                                <td class="px-6 py-4 text-sm flex space-x-2">
                                    <a href="{{ route('category.edit', $categori->category_id) }}"
                                    ...
                                    <form action="{{ route('category.destroy', $categori->category_id) }}" method="POST" class="delete-form inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
