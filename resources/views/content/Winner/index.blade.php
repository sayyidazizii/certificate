<x-app-layout>
    <div class="container mx-auto mt-6">
        ...
        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg overflow-hidden mx-5">
            ...
            <div class="overflow-x-auto mx-2">
                <table id="winners-table" class="min-w-full bg-white dark:bg-gray-800 table-auto">
                    <thead class="bg-gray-100 dark:bg-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-sm font-medium text-gray-600 dark:text-gray-200">Winner Name</th>
                    ...
                    @foreach($winners as $winner)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-200">{{ $winner->winner_name }}</td>
                            <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-200">
                                {{ $winner->created_at ? $winner->created_at->format('d M Y') : 'No date' }}
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-200">
                                {{ $winner->updated_at ? $winner->updated_at->format('d M Y') : 'No date' }}
                            </td>
                            <td class="px-6 py-4 text-sm flex space-x-2">
                                <a href="{{ route('winner.edit', $winner->id) }}"
                                ...
                                <form action="{{ route('winner.destroy', $winner->id) }}" method="POST" class="delete-form inline">
                                    @csrf
                                    @method('DELETE')
    ...
    <script>
        ...
        function confirmDelete(button) {
            if (confirm('Are you sure you want to delete this winner? This action cannot be undone.')) {
